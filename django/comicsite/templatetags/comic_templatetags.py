...
from dataproviders.DropboxDataProvider import DropboxDataProvider, \
    HtmlLinkReplacer  # TODO: move HtmlLinkReplacer to better location..
...
@register.tag(name="dicomviewer")
def render_dicomviewer(parser, token):
    usage = """Tag usage: {% visualization datapath:string
                                           width:number
                                           height:number
                                           deferredLoad:0|1
                                           extensionFilter:ext1,ext2,ext3%}
The only mandatory argument is datapath.
width/height: Size of the 2D view area.
deferredLoad: If active, user has to click on the area to load the viewer.
extensionFilter: An include filter to specify the file types which should be displayed in the file
browser.
"""
    try:
        args = parseKeyValueToken(token)
    except ValueError:
        errormsg = "Error rendering {% " + token.contents + " %}: Error parsing token. " + usage
        return TemplateErrorNode(errormsg)

    if "datapath" not in args.keys():
        errormsg = "Error rendering {% " + token.contents + " %}: datapath argument is missing." + usage
        return TemplateErrorNode(errormsg)

    return DicomViewerNode(args)


class DicomViewerNode(template.Node):
    """
    ...
    def render(self, context):
        return """
          <div class="COMICWebWorkstationButtons">
              <button id="comicViewerSetSmallSize{id}"> small </button>
              <button id="comicViewerSetLargeSize{id}"> large </button>
              <button id="comicViewerFullscreenToggle{id}"> fullscreen </button>
          </div>
          <div id="comicViewer{id}" style="width: {width}px; height:{height}px"></div>
          <script type="text/javascript">
            require(['jquery', 'comicwebworkstationwrapper'],
              function ($) {{
                var fmeViewer{id} = null;
                console.log('COMICWebWorkstationWrapper loaded')
                fmeViewer{id} = new COMICWebWorkstationWrapper("comicViewer{id}");
                var options = {{'path':'{path}',
                               'deferredLoad':{deferredLoad},
                               'extensionFilter':'{extensionFilter}',
                               'width':{width},
                               'height':{height},
                               'application': 'RemoteView2DDemo',
                               'webSocketHostName':'{webSocketHostName}',
                               'webSocketPort':{webSocketPort},
                               'urlToMLABRoot': "/static/js",
                               'ignoreDefaultStyleSheet': true}};
                fmeViewer{id}.init(options);

                $("#comicViewerSetSmallSize{id}").click(function(){{
                    fmeViewer{id}.setSmallSize()
                }})
                $("#comicViewerSetLargeSize{id}").click(function(){{
                    fmeViewer{id}.setLargeSize()
                }})
                $("#comicViewerFullscreenToggle{id}").click(function(){{
                    fmeViewer{id}.gotoFullscreen()
                }})
              }}
            );
          </script>
        """.format(id=id(self),
                   ...
                   path=self.args.get("datapath", "../../../MeVisLab/Resources/DemoData"),
                   extensionFilter=self.args.get("extensionFilter", ""),
                   deferredLoad=self.args.get("deferredLoad", "0"),
                   webSocketHostName=self.args.get("webSocketHostName", "apps.mevis.fraunhofer.de"),
                   webSocketPort=self.args.get("webSocketPort", 80))
...
class UrlParameterNode(template.Node):
    ...
    def render(self, context):
        ...
        if context['request'].GET.has_key(self.args['url_parameter']):
            ...
            error_message = "Error rendering %s: Parameter '%s' not found in request URL" % (
